...
class electronic_invoice_fields(models.Model):
	...
	def set_array_item_object(self, invoice_items):
		...
		logging.info("Type customers:" + str(typeCustomers))
		tasaITBMS = 'asas'
		...
		if invoice_items:
			for item in invoice_items:
				...
				new_item_object = {
					...
					'cantGTINComInv' : str('%.2f' % round(float(item.product_id.cantGTINComInv), 2)) if item.product_id.codigoGTINInv else '',
